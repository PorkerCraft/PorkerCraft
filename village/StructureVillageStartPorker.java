...
public class StructureVillageStartPorker extends StructureStart {
	/** well ... thats what it does */
	private boolean hasMoreThanTwoComponents = false;

	public StructureVillageStartPorker(World par1World, Random par2Random, int par3, int par4, int par5) {
		ArrayList arraylist = StructureVillagePiecesPorker.getStructureVillageWeightedPieceList(par2Random, par5);
		
		ComponentVillageStartPiecePorker componentvillagestartpiece = new ComponentVillageStartPiecePorker(par1World.getWorldChunkManager(), 0, par2Random, (par3 << 4) + 2, (par4 << 4) + 2, arraylist, par5);
		...
		while (iterator.hasNext()) {
			...
			if (!(structurecomponent1 instanceof ComponentVillageRoadPiecePorker)) {
				++l;
			}
		}
