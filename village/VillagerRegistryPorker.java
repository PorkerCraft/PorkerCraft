 ...
 *
 */
public class VillagerRegistryPorker
{
    private static final VillagerRegistryPorker INSTANCE = new VillagerRegistryPorker();
    ...
    public interface IVillageCreationHandler
    {
         ...
         * @param p5
         */
        Object buildComponent(StructureVillagePieceWeight villagePiece, ComponentVillageStartPiecePorker startPiece, List pieces, Random random, int p1,
    ...
    public interface IVillageTradeHandler
    {
         ...
         * @param recipeList
         */
        void manipulateTradesForVillager(EntityPigMen villager, MerchantRecipeList recipeList, Random random);
    }

    public static VillagerRegistryPorker instance()
    {
        return INSTANCE;
    }
     ...
     * @param random
     */
    public static void manageVillagerTrades(MerchantRecipeList recipeList, EntityPigMen villager, int villagerType, Random random)
    {
    ...
    public static Object getVillageComponent(StructureVillagePieceWeight villagePiece, ComponentVillageStartPiecePorker par0ComponentVillageStartPiece, List pieces, Random random,
            int p1, int p2, int p3, int p4, int p5)
    {
        return instance().villageCreationHandlers.get(villagePiece.villagePieceClass).buildComponent(villagePiece, par0ComponentVillageStartPiece, pieces, random, p1, p2, p3, p4, p5);
    }


    public static void addEmeraldBuyRecipe(EntityPigMen villager, MerchantRecipeList list, Random random, Item item, float chance, int min, int max)
    {
        if (min > 0 && max > 0)
        {
            EntityPigMen.villagerStockList.put(item.itemID, new Tuple(min, max));
        }
        villager.addMerchantItem(list, item.getMaxDamage(), random, chance);
    }

    public static void addEmeraldSellRecipe(EntityPigMen villager, MerchantRecipeList list, Random random, Item item, float chance, int min, int max)
    {
        if (min > 0 && max > 0)
        {
            EntityPigMen.blacksmithSellingList.put(item.itemID, new Tuple(min, max));
        }
        villager.addBlacksmithItem(list, item.getMaxDamage(), random, chance);
    }

    public static void applyRandomTrade(EntityPigMen villager, Random rand)
    {
